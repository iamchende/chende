...
/**
 * BASE64编码解码工具包
 * 底层用的ISO8859-1编码，可能对中文不够友好；反正测试时OK的
 * Base64是一种字符串编码格式，采用了A-Z，a-z，0-9，“+”和“/”这64个字符来编码原始字符（还有垫字符“=”）。
 * 一个字符本身是一个字节，也就是8位，而base64编码后的一个字符只能表示6位的信息。也就是原始字符串中的3字节的信息编码会变成4字节的信息。
 * 核心作用应该是传输数据的正确性，有些网关或系统只能使用ASCII字符。
...
public class Base64Util {
    ...
    /**
     * 二进制数据编码为URL 安全的 BASE64 字符串
     * @param bytes
     * @return
     * @throws Exception
     */
    public static String encodeURL(byte[] bytes) throws Exception {
    ...
    public static void main(String[] args) throws Exception {
		...
		System.out.println(encodeURL(str.getBytes()));
		System.out.println(new String(decodeURL(encodeURL(str.getBytes()))));
	}
}